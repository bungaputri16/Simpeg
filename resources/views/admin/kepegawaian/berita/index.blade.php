                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tanggal Post</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Judul</th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Isi</th>
                                            <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Gambar</th>
                                    ...
                                    @foreach ($berita as $news)
                                        <tr>
                                            <td
                                                ...
                                                {{ $news->created_at->format('d-m-y') }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{ $news->judul }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200" style="max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                                    {{ $news->isi }}
                                                </td>
                                            ...
                                            <td
                                                ...
                                                <div class="w-[50px] h-50px">
                                                    @if ($news->gambar)
                                                    <img src="{{ asset('berita/' . $news->gambar) }}"
                                                     alt="{{ $news->gambar }}" />
                                                    @else
                                                    <img src="{{ asset('assets/icon/notfound_image.png') }}"
                                                    alt="Not Found " />
                                                    @endif
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium ">
                                                <div class="flex justify-end">
                                                    ...
                                                    <form
                                                        action="{{ route('news.delete', ['id' => $news->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
